...
class CloudflareDNSBot:
        ...
        try:
            ...
            try:
                ...
                if zones:
                    ...
                    account_id = (
                        zone_details.account.id
                        if hasattr(zone_details, "account") and zone_details.account
                        else None
                    )
    ...
    async def get_current_cf_client(self) -> cloudflare.Cloudflare | None:
        ...
        if (
            not self.current_tenant_id
            or self.current_tenant_id not in self.tenants_cache
        ):
            return None
    ...
    async def switch_tenant_menu(
        ...
        switch_text = f"🏢 *Switch Tenant{role_info}*\n\nSelect a tenant to switch to:\n✅ = Currently active\n👤 = You are admin"
    ...
    async def handle_record_ttl(
        ...
        try:
            ...
            zone = zones.get(zone_id)
            full_name = (
                record_name if record_name == "@" else f"{record_name}.{zone.name}"
            )
        ...
        if tunnels:
            ...
            for tunnel_id, tunnel in tunnels.items():
                ...
                if hasattr(tunnel, "created_at"):
                    tunnels_text += (
                        f"   📅 Created: {tunnel.created_at.strftime('%Y-%m-%d')}\n"
                    )
                tunnels_text += "\n"
    ...
    async def view_tunnel(
        ...
        try:
            ...
            if hasattr(tunnel, "created_at"):
                tunnel_info += (
                    f"📅 **Created:** {tunnel.created_at.strftime('%Y-%m-%d %H:%M')}\n"
                )

            # Show public hostnames if any
            if (
                tunnel_config
                and hasattr(tunnel_config, "config")
                and tunnel_config.config
            ):
    ...
    async def refresh_cache_callback(
        ...
        try:
            ...
            total_domains = sum(
                len(data["domains"]) for data in self.tenants_cache.values()
            )
